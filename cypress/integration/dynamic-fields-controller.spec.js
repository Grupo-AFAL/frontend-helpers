context('DynamicFieldsController', () => {
    ...
    it('removes the selected dynamic field template', () => {
			...
			cy.get('@container').children().eq(0).as('firstTemplate')
			cy.get('@container').children().eq(1).as('secondTemplate')
			cy.get('@container').children().eq(2).as('thirdTemplate')

			cy.get('@container').children().should('have.length',3)

			cy.get('@secondTemplate').find('input[placeholder="First name"]').type('Delete')
			cy.get('@secondTemplate').find('input[placeholder="Last name"]').type('me')

			cy.get('@secondTemplate').find('a.delete').click()
