context('SelectController', () => {
  ...
  it('displays the list of available options', () => {
    cy.get('.input').click()
    cy.get('.results li').should('have.length', 6)
    cy.get('.results li:nth-child(1)').should('have.text', 'One')
    cy.get('.results li:nth-child(2)').should('have.text', 'Two')
    cy.get('.results li:nth-child(3)').should('have.text', 'Three')
    cy.get('.results li:nth-child(4)').should('have.text', 'Four')
    cy.get('.results li:nth-child(5)').should('have.text', 'Five')
    cy.get('.results li:nth-child(6)').should('have.text', 'Six')
  })

  context('using the mouse', () => {
    it('selects an item from the list', () => {
      cy.get('.input').click()
      cy.get('.results li:nth-child(2)').click()
  ...
  context('using the keyboard', () => {
    ...
    it('selects an item after going down and up', () => {
      ...
      cy.get('.results li:nth-child(5)').should('have.class', 'selected')
